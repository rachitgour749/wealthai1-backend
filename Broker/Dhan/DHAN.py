        ...
        try:
            ...
            if not token:
                ...
                if 'errorCode' in data:
                    ...
                    logger.error(error_msg)
                    return None, error_msg
                logger.error(f"Could not find access token in response. Keys: {list(data.keys())}")
...
def place_order(credentials: Dict[str, Any], order_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        dhan_payload = {
            ...
            "validity": validity,
            "securityId": order_data.get('symbol'),  # DHAN uses securityId for symbol
            "quantity": int(order_data.get('quantity', 0))
        }
        
        # Add optional fields
        if order_data.get('price') and float(order_data.get('price', 0)) > 0:
            dhan_payload['price'] = float(order_data['price'])
        ...
        dhan_payload['afterMarketOrder'] = (variety.lower() == 'amo')
