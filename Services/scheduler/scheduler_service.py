...
class SchedulerService:
    ...
    def register_strategy_job(self, strategy_name: str):
        ...
        # Determine job types and register accordingly
        # We check both generation and execution independently to allow both for one strategy
        registered = False
        ...
        if 'signal_execution' in strategy_config:
            self._register_execution_job(strategy_name, strategy_config)
            registered = True
            
        if 'data_fetch' in strategy_config:
            self._register_data_fetch_job(strategy_name, strategy_config)
            registered = True
            
        if not registered:
            logger.warning(f"No known job types found for strategy: {strategy_name}")
    ...
    def _register_execution_job(self, strategy_name: str, config: Dict):
        """
        Register a signal execution job
        
        Args:
            strategy_name: Name of the strategy
            config: Strategy configuration dictionary
        """
        sig_exec = config['signal_execution']
        
        try:
            # We use the common SignalExecutor for execution
            # But we allow the config to specify a custom one if needed
            ...
            logger.info(f"Loading signal executor: {module_path}.{function_name}")
            
            # Special handling for instance methods if needed, but for simplicity:
            # If function_name contains '()', we instantiate the class
            if '()' in function_name:
                class_name, method_name = function_name.split('().')
                module = importlib.import_module(module_path)
                executor_class = getattr(module, class_name)
                
                # Wrap the call in a function for APScheduler
                def wrapper_func():
                    instance = executor_class()
                    method = getattr(instance, method_name)
                    method()
                
                exec_func = wrapper_func
            else:
                module = importlib.import_module(module_path)
                exec_func = getattr(module, function_name)
            
            # Create cron trigger
            trigger = self._create_trigger(sig_exec)
            
            # Add job to scheduler
            job = self.scheduler.add_job(
                exec_func,
                trigger=trigger,
                id=f'execution_{strategy_name}',
                name=f'Signal Execution: {strategy_name}',
                replace_existing=True
            )
            
            self.jobs[f'execution_{strategy_name}'] = job
            logger.info(f"✓ Registered execution job: {strategy_name}")
            
        except Exception as e:
            logger.error(f"Failed to register execution job '{strategy_name}': {e}")
            raise
