...
async def save_etf_strategy(request: SaveETFStrategyRequest):
    """Save an ETF strategy to the SavedInstance table (unified model)"""
    session = None
    try:
        ...
        existing = session.query(SavedInstance).filter(
            ...
            SavedInstance.tickers == tickers_str
        ).first()
        ...
        if existing:
            return {
                ...
                "existing_strategy": {"id": existing.id, "strategy_name": existing.strategy_name},
                "strategy_exists": True
            }
            
        # Create new SavedInstance
        # Store withdrawal parameters in strategies_parameters dict
        strategy_params = {
            'capital_per_week': request.capital_per_week,
            'accumulation_weeks': request.accumulation_weeks,
            'brokerage_percent': request.brokerage_percent,
        ...
        new_instance = SavedInstance(
            ...
            start_date=request.start_date,
            end_date=request.end_date,
            strategies_parameters=strategy_params,
            backtest_results_json=request.backtest_results.dict() if request.backtest_results else {},
            status='deploy',
...
async def get_saved_etf_strategies(user_id: str):
    """Get all saved ETF payout strategies for a user"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Services.strategy_manager.models import SavedInstance
        ...
        instances = session.query(SavedInstance).filter(
            ...
            SavedInstance.strategy_type == 'Rotation_ETF_Payout'
        ).all()
        
        strategies = []
        for instance in instances:
            params = instance.strategies_parameters or {}
            strategies.append({
                "id": instance.id,
                ...
                "use_custom_dates": params.get('use_custom_dates', False),
                "withdraw_amount": params.get('withdraw_amount', 0),
                "payout_start_week": params.get('payout_start_week', 1),
...
async def get_saved_etf_strategy_by_id(strategy_id: int):
    """Get a specific saved ETF payout strategy by ID"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Services.strategy_manager.models import SavedInstance
...
async def get_saved_etf_strategies_count(user_id: str):
    """Get count of saved ETF payout strategies for a specific user"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Services.strategy_manager.models import SavedInstance
...
async def get_saved_etf_strategies_table(user_id: str):
    """Get saved ETF payout strategies in table format"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Services.strategy_manager.models import SavedInstance
...
async def stop_etf_strategy_batch(request: dict):
    """Stop an ETF payout strategy (batch/body request)"""
    session = None
    try:
...
async def restart_etf_strategy_batch(request: dict):
    """Restart an ETF payout strategy (batch/body request)"""
    session = None
    try:
        strategy_id = request.get("strategy_id")
        user_id = request.get("user_id")
...
async def stop_etf_strategy(strategy_id: int):
    """Stop a specific ETF payout strategy by ID"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Services.strategy_manager.models import SavedInstance
...
async def restart_etf_strategy(strategy_id: int):
    """Restart a specific ETF payout strategy by ID"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Services.strategy_manager.models import SavedInstance
...
async def get_recent_etf_signals(days: int = 7):
    """Get recent ETF payout signals from TradingSignal table"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        ...
        signals = session.query(TradingSignal).filter(
            and_(
                TradingSignal.strategy_type == 'Rotation_ETF_Payout',
...
async def get_etf_signals_by_run_id(run_id: str):
    """Get ETF signals for a specific run_id from TradingSignal table"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Databases.signal_models import TradingSignal
...
async def get_etf_signal_runs(days: int = 30):
    """Get list of ETF payout signal runs from TradingSignal table"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Databases.signal_models import TradingSignal
...
async def get_latest_etf_signals():
    """Get the latest ETF payout signals from TradingSignal table"""
    session = None
    try:
        from Databases.app_data_db_connection import get_session
        from Databases.signal_models import TradingSignal
        ...
        session = get_session()
        # Find latest run_id for this strategy type
