...
class stockRotationBacktester:
    ...
    def execute_capital_raising_process(self, week_num: int, execution_date: datetime, 
        ...
        # NEW LOGIC: Continue selling from same stock until target reached or stock exhausted
        for _, stock_row in sorted_for_sell.iterrows():
            ...
            if ticker not in updated_holdings or updated_holdings[ticker] <= 0:
                print(f"⚠️ No holdings in {ticker} - skipping to next stock")
                continue
            ...
            # CONTINUOUS SELLING LOOP - Keep selling from same stock until target reached or stock exhausted
            ...
            # Check if we need to move to next stock
            if total_raised < target_capital and updated_holdings.get(ticker, 0) == 0:
                print(f"📦 {ticker} exhausted - moving to next priority stock")
