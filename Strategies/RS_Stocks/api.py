...
class StrategyConfigCreate(BaseModel):
    config_name: str
    main_index: str = "^NSEI"
    stock_universe: Optional[str] = None  # Add missing stock_universe field
...
class BacktestRequest(BaseModel):
    ...
    custom_stocks: Optional[List[str]] = None  # Custom stock selection (Option B: stock_universe + custom_stocks)
    max_positions: int = 20
...
async def run_backtest(backtest_request: BacktestRequest, user_id: str = Query(..., description="User ID for the backtest"), db: Session = Depends(get_db)):
    ...
    config_dict = {
        ...
        'stock_universe': backtest_request.stock_universe,
        'custom_stocks': backtest_request.custom_stocks,  # Add custom stock selection
    ...
    try:
        ...
        backtester = RSStrategyBacktester.from_config_dict(db, config_dict)
...
@router.get("/stocks/universe/{universe}")
async def get_stocks_by_universe(universe: str, db: Session = Depends(get_db)):
    """Get stocks for a specific universe (NIFTY50, NIFTY100, NIFTY200, NIFTY500)"""
    try:
        # Validate universe
        valid_universes = ['NIFTY50', 'NIFTY100', 'NIFTY200', 'NIFTY500']
        if universe.upper() not in valid_universes:
            raise HTTPException(status_code=400, detail=f"Invalid universe. Must be one of: {valid_universes}")
        
        universe = universe.upper()
        
        # Create a temporary backtester instance to access stock methods
        temp_config = {
            'main_index': '^NSEI',
        ...
        # Get stocks for the universe
        stocks = backtester.get_custom_stock_universe()
        
        # Format response with stock details
        stock_list = []
        ...
        logger.error(f"Error fetching stocks for universe {universe}: {e}")
...
async def calculate_rs_strategy_date_range(request: Dict[str, Any], db: Session = Depends(get_db)):
    """Calculate common date range for selected stocks for RS Strategy"""
    try:
...
async def get_saved_strategies_list(user_id: str, db: Session = Depends(get_db)):
    """Get all saved RS Stock strategies for a specific user"""
    try:
        strategies = db.query(SavedRSStrategy).filter(SavedRSStrategy.user_id == user_id).order_by(SavedRSStrategy.created_at.desc()).all()
        ...
        return {
            "success": False,
            "message": f"Error fetching RS Stock strategies: {str(e)}"
        }
