...
=======
    ...
    if is_port_in_use(port):
        ...
        if kill_process_on_port(port):
            print(f"[OK] Port {port} has been freed")
        else:
            print(f"[FAIL] Could not free port {port}. Trying alternative port...")
>>>>>>> feature/chatai
...
=======
                ...
                print("[FAIL] No available ports found. Please manually stop processes using ports 8000-8005")
>>>>>>> feature/chatai
                return False
...
=======
        print(f"[OK] Server starting at http://127.0.0.1:{port}")
        subprocess.run([sys.executable, 'server.py'], env={**os.environ, 'PORT': str(port)})
    except KeyboardInterrupt:
